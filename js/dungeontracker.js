(function(window) {
	...
	window.updateOverworldShuffle = function(showVanillaOWBox)
	{
		...
		document.getElementById("vanillawelcome").style.display = layoutshuffle === 'N' && crossedow !== 'C' ?"none" :"block";
		updateOverviewElements();
		sendUpdate();
	...
	window.loadAllTrack = function(newData)
	{
		...
		document.getElementById("overworldoptionsfinalbox").style.display = newData.showOptionsFinalBox && (layoutshuffle !== 'N' || whirlpoolshuffle || crossedow !== 'N' || mixedow) ?"block" :"none";
		if(document.getElementById("globalsync").checked)
		{
	...
	window.clearData = function()
	{
		if(confirm("Delete all currently loaded paths and notes for every dungeon and the overworld?"))
		{
			...
			pinnedPaths = [];
			previousPaths = [];
			document.getElementById("overworldoptionsfinalbox").style.display = layoutshuffle !== 'N' || whirlpoolshuffle || crossedow !== 'N' || mixedow ?"block" :"none";
			updateOverviewElements();
			updateReachableEdges();
			updateItemTracker();
			updateConnectorList();
	...
	window.start = function()
	{
		...
		document.getElementById("excludebigkeys").checked = excludebigkeys;
		document.getElementById("layoutshuffle").value = ""+layoutshuffle;
		document.getElementById("whirlpoolshuffle").checked = false;
		...
		document.getElementById("vanillawelcome").style.display = layoutshuffle === 'N' ?"none" :"block";
		if(sync)
			document.getElementById("itemsync").disabled = document.getElementById("connectorsync").disabled = document.getElementById("selectworldstate").disabled = document.getElementById("entranceenabled").disabled = true;
		if(!query.request_update && (layoutshuffle !== 'N' || whirlpoolshuffle || crossedow !== 'N' || mixedow))
			document.getElementById("overworldoptionsfinalbox").style.display = "block";
		try
		{
	...
	window.drawFullOverworldPanel = function(main)
	{
		...
		for(let [id,screen] of overworldScreens)
		{
			...
			if((id === 0x18 || (id === 0x58 && mapMode === "single")) && !fullOWConnectorStart && ownItems.flute && !document.getElementById("activeflutebox").checked)
			...
			for(let k = 0; k < fullOWPath.path.length; k += 3)
			{
				if(k != 0 && !specialEdgeTypes.includes(fullOWPath.path[k-1]))
				{
					...
					if(mapMode === "vertical" && isDarkWorld(screen1) != drawDarkWorld)
						y += 1024*scale;
					s += drawFullEdgeConnection(edge1,edge2,x,y,mapMode != "single" || isDarkWorld(screen1) === isDarkWorld(screen2) ?1 :.5);
	...
	window.hoverScreen = function(id,main)
	{
		...
		for(let edge of screen.edges.values())
			if(edge.out && (edge.string === "ZW" ?whirlpoolshuffle :(layoutshuffle !== 'N' || (crossedow === 'C' && edge.parallel))))
				s += drawFullEdgeConnection(edge,edge.out,x,y,(main ?mapModeMain !== "single" :mapModePopout !== "single") || isDarkWorld(screen) === isDarkWorld(edge.out.screen) ?1 :.5,main);
